...
var CommonConfig = map[string]string{
	"log_debug": config.GetCommonConf()["Log_debug"],
	"timezone": config.GetCommonConf()["timezone"], // 时区
}

// Log 自定义终端打印日志
func Log(txt ...interface{}) {
	if CommonConfig["log_debug"] == "true" {
		log.Println("Log：", txt)
	}
}
...
func GetTimeS() int64 {
	// 时区
	log.Println(CommonConfig["timezone"])
	timeZone, _ := time.LoadLocation(CommonConfig["log_debug"])
	//timeZone := time.FixedZone("CST", 8*3600) // 东八区
...
func GetTimeMS() int64 {
	// 时区
	timeZone, _ := time.LoadLocation(CommonConfig["timezone"])
	//timeZone := time.FixedZone("CST", 8*3600) // 东八区
